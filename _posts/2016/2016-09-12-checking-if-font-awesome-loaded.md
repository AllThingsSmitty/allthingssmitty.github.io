...
Icon font libraries, unlike JavaScript libraries, typically consist of only CSS and fonts files. [Font Awesome](http://fontawesome.io/){:target="_blank"}{:rel="noopener noreferrer"} is among the most popular of these resources, so we'll use that for this example.
...
In order to be able to read the `font-family` property we need a way to get the values of the `<span>`. The [`getComputedStyle()`](https://developer.mozilla.org/en-US/docs/Web/API/Window/getComputedStyle){:target="_blank"}{:rel="noopener noreferrer"} method will return all CSS values of an element, which is perfect for what we're doing.
